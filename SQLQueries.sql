...
[dbo].[Subject].subject_name, COUNT([dbo].[ResultInfo].subject_id) AS stud_num
FROM [dbo].[Subject] JOIN [dbo].[ResultInfo] ON [dbo].[Subject].subject_id = [dbo].[ResultInfo].subject_id
WHERE [dbo].[ResultInfo].result >= 150 AND [dbo].[ResultInfo].session_name = 'îñíîâíà'
GROUP BY subject_name


--рейтинг популярності предметів
SELECT
...
INSERT INTO [dbo].[Student] ([full_name], [location_id]) VALUES ('Êîëîäüêî Î. À.', 1);
DELETE FROM [dbo].[Student] WHERE [full_name] = 'Êîëîäüêî Î. À.';
